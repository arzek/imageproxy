...
// 	http://localhost/http://example.com/image.jpg
func NewRequest(r *http.Request, baseURL *url.URL, mongoClient *mongo.Client) (*Request, error) {
	var err error
	req := &Request{Original: r}

	path := r.URL.EscapedPath()[1:] // strip leading slash

	collection := mongoClient.Database("manga").Collection("images")

	var result struct {
		UrlOriginal string
	}
	filter := bson.M{"url": "/" + path}
	err = collection.FindOne(r.Context(), filter).Decode(&result)
	if err != nil {
		log.Fatal(err)
	}

	path = result.UrlOriginal
